...
@tool
def get_top_bottom_agents(metric: str, n: int = 3,
                          period: str = "all",
                          rank: str = "top") -> str:
    """
    Get top or bottom N agents for a specific KPI.
    Use for ranking and leaderboard questions.
    Args:
        metric: MUST be exactly one of: aht, fcr, csat, adherence
        n: number of agents to return (default 3)
        period: Q1, Q2, Q3, Q4, specific month like Jan-2024, or all
    ...
    if not metric or not str(metric).strip():
        return (
            "Metric is required. "
            "Please specify one of: aht, fcr, csat, adherence"
        )

    metric = str(metric).lower().strip().strip('"').strip("'")

    # Handle natural language variations the LLM might send
    metric_aliases = {
        ...
        "satisfaction":             "csat",
        "schedule adherence":       "adherence",
        "adherence rate":           "adherence",
        "schedule":                 "adherence",
    }
    metric = metric_aliases.get(metric, metric)

    # Validate metric is one of the 4 valid options
    valid_metrics = ["aht", "fcr", "csat", "adherence"]
    ...
    # Clean rank input
    if not rank or not str(rank).strip():
        rank = "top"
    rank = str(rank).lower().strip().strip('"').strip("'")
    ...
    if not period or not str(period).strip():
        period = "all"
    period = str(period).strip().strip('"').strip("'")

    # ── SORT DIRECTION ─────────────────────────────────────────────
    # AHT: lower is better → top performers = lowest AHT → ASC
    # FCR, CSAT, Adherence: higher is better → top = highest → DESC
    if metric == "aht":
        order = "ASC" if rank == "top" else "DESC"
    else:
    ...
    try:
        ...
        if period.upper() in QUARTERS:
            ...
            rows = conn.execute(f"""
                SELECT agent_name,
                       team,
                       ROUND(AVG({metric}), 2) AS v
                FROM performance
                WHERE month_year IN ({ph})
    ...
    if not rows:
        return (
            ...
            f"in period '{period}'. "
            "Check the period format e.g. Q1, Q2, Jan-2024, or all"
        )
    ...
    rank_word = "Top" if rank == "top" else "Bottom"
    target = {
        "aht": "Target: < 5.0 min",
        "fcr": "Target: > 80%",
        "csat": "Target: > 85%",
    ...
    out  = f"{rank_word} {n} agents by {label}\n"
    out += f"Period: {period} | {target}\n\n"
    ...
    for i, row in enumerate(rows, 1):
        ...
        # Flag agents above/below target
        target_vals = {
            "aht": 5.0, "fcr": 80.0,
            "csat": 85.0, "adherence": 90.0
        }
        t = target_vals.get(metric, 0)

        if metric == "aht":
            flag = "✅" if value <= t else "⚠️"
        else:
            flag = "✅" if value >= t else "⚠️"

        out += (
            f"  {i}. {flag} {agent_name} ({team}): "
            f"{value} {unit}\n"
        )
